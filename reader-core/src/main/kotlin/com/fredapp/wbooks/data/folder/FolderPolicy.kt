...
/**
 * Folder limits are deliberately small enough for a round watch UI and large
 * enough for real libraries. Folders are top-level only across watch, web, and
 * phone utility.
 */
object FolderPolicy {
    ...
    fun validateMoveTarget(raw: String, existingFolders: Collection<String>): Validation {
        val base = validateName(raw, allowRoot = true)
        val name = base.name ?: return base
        if (name.isEmpty()) return Validation("", null)
        val existing = existingFolders.firstOrNull { it.equals(name, ignoreCase = true) }
        if (existing == null) {
            return Validation(null, "Folder does not exist")
        }
        return Validation(existing, null)
    }
